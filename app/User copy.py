...
async def get_user_pages(request: Request):
    data=parse_request_body(request, QueryCondition)
   ...
   # 替换原有赋值逻辑
    ...
    if data.phone:
        query = query.filter(phone=data.phone)
    if data.userId:
    ...
    UserItem(
        ...
        createdTime=user.createdTime.isoformat() if user.createdTime else None,
...
async def saveOrUpdate(request: Request):
    User_in=parse_request_body(request, Userin)
    if User_in.id is None:
        try:
